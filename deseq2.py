...
annotations=jawm.Process(
    name="annotations",
    when=lambda p: not os.path.isfile( os.path.join( p.var["project_folder"], "deseq2_output" , "biotypes_go.txt") ) ,
    script="""\
...
""",
    ...
    manager_slurm={ "-c": 1, "--mem": "8GB", "-t": "1h" }
)
...
""",
    ...
    container="mpgagebioinformatics/deseq2:1.38.0",
    manager_slurm={ "-c": 20, "--mem": "20GB", "-t": "1h" }
)
...
""",
    ...
    container="mpgagebioinformatics/deseq2:1.38.0",
    manager_slurm={ "-c": 8, "--mem": "40GB", "-t": "4h" }
)
...
""",
    ...
    container="mpgagebioinformatics/rnaseq.python-3.8-8",
    manager_slurm={ "-c": 1, "--mem": "8GB", "-t": "12h" }
)

topgo_proc=jawm.Process(
    name="topgo_proc",
...
library(openxlsx)
## ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
fin="{{project_folder}}/deseq2_output/annotated/{{input_file}}.tsv" # topGO.tsv
...
""",
    ...
    manager_slurm={ "-c": 1, "--mem": "8GB", "-t": "4h" }
)
...
""",
    ...
    manager_slurm={ "-c": 8, "--mem": "16GB", "-t": "4h" }
)

rcistarget_proc=jawm.Process(
    name="rcistarget_proc",
...
""",
    ...
    container="mpgagebioinformatics/rnaseq.python-3.8-8",
    manager_slurm={ "-c": 8, "--mem": "20GB", "-t": "4h" }
)
...
if __name__ == "__main__":
    ...
    workflows, var, args, unknown_args = jawm.utils.parse_arguments(['main','nf_deseq2','test'])

    if workflow(["main","deseq2","test"], workflows):

        get_images.execute()
        tx2gene.execute(get_images.hash)
        annotations.execute(tx2gene.hash)
        parse_submission.execute(annotations.hash)
        tx2gene_proc.execute(parse_submission.hash)
        deseq2.execute(tx2gene_proc.hash)
        mastertable.execute(deseq2.hash)
        annotator.execute(mastertable.hash)
        david_proc.execute(annotator.hash)
        topgo_proc.execute(david_proc.hash)
        cellplot.execute(topgo_proc.hash)
        rcistarget_proc.execute(cellplot.hash)
        qc_plots.execute(rcistarget_proc.hash)
        get_ip.execute(qc_plots.hash)
        string.execute(get_ip.hash)
        release_ip.execute(string.hash)
        upload_paths.execute(release_ip.hash)
        jawm.Process.wait()
